...
This tokenizer fits T5-style text-to-text modeling.
"""
...
class AlmondUnigramTokenizer:
    ...
    def __init__(
        ...
        if not isinstance(unk_log_prob, float):
            raise TypeError(
                f"unk_log_prob must be float, got {type(unk_log_prob).__name__}"
            )

        if unk_log_prob >= 0:
            raise ValueError(
                f"unk_log_prob should be negative log probability, got {unk_log_prob}"
            )
    ...
    def encode(
        ...
        if not isinstance(add_eos, bool):
            raise TypeError(f"add_eos must be bool, got {type(add_eos).__name__}")
        ...
        token_ids: list[int] = []
        append_token_ids = token_ids.append

        assert self.unk_token_id is not None, (
            "unk_token_id should not be None after initialization"
        )
        assert self.eos_token_id is not None, (
            "eos_token_id should not be None after initialization"
        )
    ...
    def load(
        ...
        self.whitespace_marker = str(state["whitespace_marker"])
        self.num_extra_ids = int(state["num_extra_ids"])

        self.pad_token = str(state["pad_token"])
        self.unk_token = str(state["unk_token"])
        self.eos_token = str(state["eos_token"])
        self.special_tokens = [str(token) for token in state["special_tokens"]]
    ...
    def _validate_loaded_state(self) -> None:
        ...
        for token in self.special_tokens:
            if token not in self.piece_to_id:
                raise ValueError(
                    f"special token {token!r} is missing from loaded piece_to_id"
                )
        ...
        for token, expected_id in expected_ids.items():
            actual_id = self.piece_to_id.get(token)
        ...
        if self.decoder_start_token_id != self.pad_token_id:
            raise ValueError(
                f"decoder_start_token_id should be the same as pad_token_id in loaded state: "
                f"expected {self.pad_token_id}, got {self.decoder_start_token_id}"
            )
    ...
    def _build_seed_vocab_from_texts(
        ...
        for text in tqdm(texts, desc="Building seed vocab", unit="line"):
            if not isinstance(text, str):
                raise TypeError(
        ...
        if not piece_counts:
            raise ValueError(
                "cannot build seed vocab from empty texts; no candidate pieces found."
            )
    ...
    def _initialize_vocabulary(
        ...
        if total_count <= 0:
            raise ValueError(
                f"total normal piece count must be positive, got {total_count}"
            )
        ...
        self.pad_token_id = self.piece_to_id.get(self.pad_token)
        self.unk_token_id = self.piece_to_id.get(self.unk_token)
        self.eos_token_id = self.piece_to_id.get(self.eos_token)
    ...
    def _get_piece_log_prob(self, piece: str) -> float | None:
        ...
        return self.piece_log_probs.get(piece)
    ...
    def _viterbi_segment(
        ...
        """Find the best segmentation for normalized_text using Viterbi DP."""
        self._require_initialized()
        ...
        if dp[n] == float("-inf"):
            raise RuntimeError(
                "failed to segment text with current vocabulary. "
        ...
        while position > 0:
            ...
            if prev is None:
                raise RuntimeError(
                    "broken Viterbi backpointer state. "
    ...
    def _forward_backward_expected_counts(
        ...
        edges_by_start: list[list[tuple[int, str, float]]] = [
            [] for _ in range(n + 1)
        ]
        ...
        if log_likelihood == float("-inf"):
            raise RuntimeError(
                "failed to compute forward probabilities; no valid segmentations. "
                f"normalized_text={normalized_text!r}"
            )
        ...
        for start in range(n):
            ...
            for end, piece, log_prob in edges_by_start[start]:
                ...
                posterior_log_prob = (
                    alpha[start] + log_prob + beta[end] - log_likelihood
                )
    ...
    def _em_step(
        ...
        if not isinstance(smoothing, float):
            raise TypeError(
                f"smoothing must be float, got {type(smoothing).__name__}"
            )
        ...
        for text in tqdm(
            ...
            for piece, count in expected_counts.items():
                total_expected_counts[piece] = (
                    total_expected_counts.get(piece, 0.0) + count
                )
    ...
    def _prune_vocabulary(
        ...
        for piece, _log_prob in sorted_pieces:
    ...
    def train(
        ...
        if not isinstance(num_em_steps, int):
            raise TypeError(
                f"num_em_steps must be int, got {type(num_em_steps).__name__}"
            )

        if not isinstance(shrinking_factor, float):
            raise TypeError(
                f"shrinking_factor must be float, got {type(shrinking_factor).__name__}"
            )
        ...
        print(f"Seed vocab size: {len(seed_vocab)}")
